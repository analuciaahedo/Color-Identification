...
def blue_percentage(self, image, mask):
...
class CVExample(Node):
    def __init__(self):
        ...
        self.pub = self.create_publisher(Image, 'img_properties/blue/msk', 10)
    ...
    def timer_callback(self):
        try:
            if self.valid_img:
                ...
                three_channel_gray[:, :, 0] = np.maximum(eroded_image[:, :, 0], three_channel_gray[:, :, 0])  # Combina el rojo con gris

                self.pub.publish(self.bridge.cv2_to_imgmsg(three_channel_gray, "bgr8"))
                self.get_logger().info(f'Percentage of blue pixels: {blue_percentage}')
