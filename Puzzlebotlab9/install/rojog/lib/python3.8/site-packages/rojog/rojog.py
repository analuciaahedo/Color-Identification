...
import numpy as np
import cv2
from cv_bridge import CvBridge000
...
class CVExample(Node):
    def __init__(self):
        ...
        self.lower = np.array([0, 100, 100], np.uint8) 
    ...
    def timer_callback(self):
        try:
            if self.valid_img:
               ...
               gray_with_red = cv2.bitwise_or(gray, red_mask)
              ...
              # bgr_image = cv2.cvtColor(hsvFrame, cv2.COLOR_HSV2BGR)
               ...
               self.pub.publish(self.bridge.cv2_to_imgmsg(gray_with_red, "mono8"))
        except:
